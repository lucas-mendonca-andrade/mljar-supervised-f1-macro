...
def negative_f1_macro(y_true, y_predicted, sample_weight=None):
    ...
    val = f1_score(y_true, y_predicted, sample_weight=sample_weight, average="macro")
    return -val
...
def xgboost_eval_metric_f1_macro(preds, dtrain):
    target = dtrain.get_label()
    weight = dtrain.get_weight()
    if len(weight) == 0:
        weight = None
    ...
    return "f1_macro", negative_f1_macro(target, preds, weight)
...
def lightgbm_eval_metric_f1_macro(preds, dtrain):
    target = dtrain.get_label()
    weight = dtrain.get_weight()
    unique_targets = np.unique(target)
    if len(unique_targets) > 2:
        ...
        rows = int(preds.shape[0] / len(unique_targets))
        preds = np.reshape(preds, (rows, cols), order="F")
    return "f1_macro", -negative_f1_macro(target, preds, weight), True
...
class Metric(object):
    def __init__(self, params):
        ...
        elif self.name == "f1_macro":
            self.metric = negative_f1_macro
